...
import { DEFAULT_CATEGORIES, DEFAULT_ACCOUNTS, PlusIcon, EditIcon, TrashIcon, SparklesIcon, CheckIcon, CogIcon, ChartPieIcon, ListBulletIcon, CreditCardIcon, BuildingLibraryIcon, APP_NAME, XIcon, MenuIcon } from './constants';
...
const GastoZenProvider: React.FC<{children: ReactNode}> = ({ children }) => {
  ...
  const updateTransaction = useCallback((updatedTransaction: Transaction) => {
    ...
    // Validación de fondos para la transacción actualizada si es un gasto
    if (updatedTransaction.type === 'expense') {
      const targetAccount = accounts.find(a => a.id === updatedTransaction.accountId);
      if (targetAccount) {
        let effectiveBalance = targetAccount.balance;
        // Si la transacción original afectó a esta misma cuenta, calculamos el saldo como si se revirtiera
        if (originalTransaction.accountId === updatedTransaction.accountId) {
          if (originalTransaction.type === 'income') {
            effectiveBalance -= originalTransaction.amount; // Se quita el ingreso original
          } else { // Gasto original
            effectiveBalance += originalTransaction.amount; // Se devuelve el gasto original
          }
        }
        // Ahora, ¿el nuevo gasto haría que esta cuenta quede en negativo?
        ...
        return; // No se puede validar si la cuenta no existe
      }
    }
    ...
    // 2. Aplicar el efecto de la transacción actualizada
...
const Layout: React.FC<{children: ReactNode, isSidebarOpen: boolean, toggleSidebar: () => void, closeSidebar: () => void}> = ({ children, isSidebarOpen, toggleSidebar, closeSidebar }) => {
  ...
  }, [location.pathname]); // Only trigger on location change, not full location object to avoid re-runs on hash changes if any
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <main className="flex-1 flex flex-col md:ml-64 transition-all duration-300 ease-in-out min-w-0"> {/* Added min-w-0 for better flex handling */}
        {/* Top bar for mobile menu button and page title */}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={initialTransaction ? "Editar Transacción" : "Agregar Nueva Transacción"} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700 mt-6"> {/* Added border and margin for separation */}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        {/* Button is part of the page header on desktop, title is in top bar on mobile */}
        <div className="hidden md:block">
         ...
         <div className="md:hidden fixed bottom-6 right-6 z-30"> {/* FAB for mobile */}
...
const isLight = (color: string): boolean => {
  ...
  if (hex.length !== 3 && hex.length !== 6) return true; // Invalid hex
  let r, g, b;
  if (hex.length === 3) {
...
const SettingsPage: React.FC = () => {
    const { getAccountById, getCategoryById } = useGastoZen();

    const handleExport = () => {
        const currentTransactions = JSON.parse(localStorage.getItem(LS_TRANSACTIONS) || '[]') as Transaction[];
        const currentAccounts = JSON.parse(localStorage.getItem(LS_ACCOUNTS) || '[]') as Account[];
        const currentCategories = JSON.parse(localStorage.getItem(LS_CATEGORIES) || '[]') as Category[];

        const transactionsToExport = currentTransactions.map(t => ({
            ...t,
            categoryName: getCategoryById(t.categoryId)?.name || 'N/A',
            accountName: getAccountById(t.accountId)?.name || 'N/A',
        }));
        
        const wb = XLSX.utils.book_new();
        const wsTransactions = XLSX.utils.json_to_sheet(transactionsToExport);
        const wsAccounts = XLSX.utils.json_to_sheet(currentAccounts);
        const wsCategories = XLSX.utils.json_to_sheet(currentCategories);
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const importedAccounts = XLSX.utils.sheet_to_json<Account>(accountsSheet);
                    const importedCategories = XLSX.utils.sheet_to_json<Category>(categoriesSheet);
                    const importedTransactionsRaw = XLSX.utils.sheet_to_json<any>(transactionsSheet);

                    if (!Array.isArray(importedAccounts) || !Array.isArray(importedCategories) || !Array.isArray(importedTransactionsRaw)) {
                         alert("Error al leer los datos de las hojas del Excel.");
                         return;
                    }
                    
                    const validatedAccounts = importedAccounts.map(acc => ({
                        ...acc,
                        balance: Number(acc.balance) || 0,
                    }));

                    const validatedCategories = importedCategories.map(cat => ({
                        ...cat,
                         type: (cat.type === 'income' || cat.type === 'expense') ? cat.type : 'expense',
                    }));
                    
                    const validatedTransactions = importedTransactionsRaw.map(rawTx => {
                        let dateStr = rawTx.date;
                        if (rawTx.date instanceof Date) {
                            dateStr = rawTx.date.toISOString().split('T')[0];
                        } else if (typeof rawTx.date === 'number') { 
                             const excelEpoch = new Date(1899, 11, 30);
                             const jsDate = new Date(excelEpoch.getTime() + rawTx.date * 24 * 60 * 60 * 1000);
                             dateStr = jsDate.toISOString().split('T')[0];
                        } else if (typeof rawTx.date === 'string') {
                             if (!/^\d{4}-\d{2}-\d{2}$/.test(rawTx.date)) {
                                const parsed = new Date(rawTx.date);
                        ...
                        if (!categoryId && rawTx.categoryName) {
                            const foundCat = validatedCategories.find(c => c.name === rawTx.categoryName && c.type === rawTx.type);
                            if (foundCat) categoryId = foundCat.id;
                        }

                        let accountId = rawTx.accountId;
                        if (!accountId && rawTx.accountName) {
                            const foundAcc = validatedAccounts.find(a => a.name === rawTx.accountName);
                            if (foundAcc) accountId = foundAcc.id;
                        }
                        
                        if (!validatedCategories.find(c => c.id === categoryId)) {
                            categoryId = validatedCategories.find(c => c.type === rawTx.type)?.id || validatedCategories[0]?.id;
                        }
                         if (!validatedAccounts.find(a => a.id === accountId)) {
                            accountId = validatedAccounts[0]?.id;
                        }


                        return {
                            id: String(rawTx.id || `imported-${Date.now()}-${Math.random()}`),
                            date: dateStr,
                            description: String(rawTx.description || 'N/A'),
                            amount: Number(rawTx.amount) || 0,
                            type: (rawTx.type === 'income' || rawTx.type === 'expense') ? rawTx.type : 'expense',
                            categoryId: categoryId || validatedCategories[0]?.id, 
                            accountId: accountId || validatedAccounts[0]?.id,   
                            notes: String(rawTx.notes || ''),
                        } as Transaction;
                    }).filter(t => t.categoryId && t.accountId); 

                    if (window.confirm("Importar datos desde Excel sobrescribirá los datos existentes. ¿Estás seguro?")) {
                        localStorage.setItem(LS_ACCOUNTS, JSON.stringify(validatedAccounts));
                        localStorage.setItem(LS_CATEGORIES, JSON.stringify(validatedCategories));
                        localStorage.setItem(LS_TRANSACTIONS, JSON.stringify(validatedTransactions));
